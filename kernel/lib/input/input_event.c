...
void input_event_append_event(input_event_t event) {
    ...
    buffer_pointer += (buffer_pointer + 1) % 255; 
}

/**
 * @return input_event_t* pointer to event in buffer last-read buffer pointer
 */
input_event_t* input_event_get_event() {
    if (buffer_pointer == last_read_buffer_pointer) {
        return NULL;
    }
    last_read_buffer_pointer = (last_read_buffer_pointer + 1) % 255;
    return &buffer[last_read_buffer_pointer];
