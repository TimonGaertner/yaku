...
uint64_t isr_syscall(uint64_t rid, uint64_t rsi, uint64_t rdx, uint64_t rcx, uint64_t r8,
                     uint64_t r9) {
    ...
    case SYS_create_virtual_file:
        return syscall_create_virtual_file((struct fuse_operations*)rsi,
                                           (enum endpoint_type)rdx, (char*)rcx);
        break;
